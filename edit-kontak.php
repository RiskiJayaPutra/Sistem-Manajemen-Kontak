...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] == UPLOAD_ERR_OK) {
        ...
        if (!in_array($_FILES['foto']['type'], $allowed_types)) {
            ...
            if (!empty($kontak['foto']) && file_exists($kontak['foto'])) {
                unlink($kontak['foto']);
            }
    ...
    $kontak = array_merge($kontak, $form_data);
}
?>
<!DOCTYPE html>
<html lang="id">
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Edit Kontak</p>
                    </div>
                </div>
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 animate-slideInRight">
                <div class="bg-white rounded-2xl shadow-xl p-6 sticky top-24 border-2 border-blue-200">
                    <div class="text-center mb-6">
                        <div class="w-32 h-32 mx-auto mb-4 rounded-full bg-blue-700 flex items-center justify-center shadow-2xl overflow-hidden">
                        ...
                        <h3 class="text-2xl font-bold text-blue-900 mb-2"><?php echo htmlspecialchars($kontak['nama_lengkap']); ?></h3>
                        <span class="inline-block bg-blue-100 text-blue-700 px-4 py-2 rounded-full text-sm font-semibold">
                            <?php echo htmlspecialchars($kontak['kategori'] ?? 'Tanpa Kategori'); ?>
                        </span>
                    </div>
                    
                    <div class="space-y-3 border-t pt-4">
                        <div class="flex items-center text-gray-700">
                            <i class="fas fa-phone text-blue-700 w-8"></i>
                            <span class="text-sm"><?php echo htmlspecialchars($kontak['nomor_telepon']); ?></span>
                        </div>
                        <?php if (!empty($kontak['email'])): ?>
                            <div class="flex items-center text-gray-700">
                                <i class="fas fa-envelope text-blue-700 w-8"></i>
                                <span class="text-sm"><?php echo htmlspecialchars($kontak['email']); ?></span>
                            </div>
                        <?php endif; ?>
                        <div class="flex items-center text-gray-700">
                            <i class="fas fa-calendar text-blue-700 w-8"></i>
                            <span class="text-sm">Dibuat: <?php echo date('d/m/Y', strtotime($kontak['tanggal_dibuat'])); ?></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="mb-8 animate-fadeIn">
                    <div class="bg-blue-700 rounded-2xl shadow-xl p-6 border-4 border-blue-900 text-white">
                        <h2 class="text-3xl font-bold">
                            <i class="fas fa-edit mr-3"></i>Edit Kontak
                        </h2>
                        <p class="text-blue-100 mt-2">Perbarui informasi kontak di bawah ini</p>
                    </div>
                </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="transform transition-all duration-300 hover:scale-105">
                                    ...
                                    <input type="text" 
                                           name="nama_lengkap" 
                                           value="<?php echo htmlspecialchars($kontak['nama_lengkap']); ?>"
                                           required
                                ...
                                <div class="transform transition-all duration-300 hover:scale-105">
                                    ...
                                    <select name="kategori" 
                                        ...
                                        <option value="Keluarga" <?php echo $kontak['kategori'] == 'Keluarga' ? 'selected' : ''; ?>>👨‍👩‍👧‍👦 Keluarga</option>
                                        <option value="Teman" <?php echo $kontak['kategori'] == 'Teman' ? 'selected' : ''; ?>>👥 Teman</option>
                                        <option value="Kerja" <?php echo $kontak['kategori'] == 'Kerja' ? 'selected' : ''; ?>>💼 Kerja</option>
                                        <option value="Bisnis" <?php echo $kontak['kategori'] == 'Bisnis' ? 'selected' : ''; ?>>🤝 Bisnis</option>
                                        <option value="Lainnya" <?php echo $kontak['kategori'] == 'Lainnya' ? 'selected' : ''; ?>>📌 Lainnya</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section border-t pt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="transform transition-all duration-300 hover:scale-105">
                                    ...
                                    <input type="tel" 
                                           name="nomor_telepon" 
                                           value="<?php echo htmlspecialchars($kontak['nomor_telepon']); ?>"
                                           required
                                           class="w-full px-4 py-3 border-2 border-blue-300 rounded-lg focus:outline-none focus:border-blue-700 transition-all">
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="transform transition-all duration-300 hover:scale-105">
                                    ...
                                    <textarea name="alamat" 
                                              rows="4"
                                              class="w-full px-4 py-3 border-2 border-blue-300 rounded-lg focus:outline-none focus:border-blue-700 transition-all"><?php echo htmlspecialchars($kontak['alamat'] ?? ''); ?></textarea>
                                </div>
                                
                                <div class="transform transition-all duration-300 hover:scale-105">
                                    <label class="block text-gray-700 font-semibold mb-2">
                                        <i class="fas fa-camera mr-2 text-blue-700"></i>Ganti Foto Profil
                                    </label>
                                    <div class="border-2 border-dashed border-blue-300 rounded-lg p-4 text-center hover:border-blue-700 transition-colors">
                                        ...
                                        <label for="foto" class="cursor-pointer">
                                            ...
                                            <p class="text-gray-600 text-sm">Klik untuk upload foto baru</p>
                                            <p class="text-xs text-gray-400 mt-1">JPG, PNG, GIF (Max 2MB)</p>
                                        </label>
                                        <div id="preview" class="mt-4 <?php echo !empty($kontak['foto']) && file_exists($kontak['foto']) ? '' : 'hidden'; ?>">
                                            <img src="<?php echo htmlspecialchars($kontak['foto'] ?? ''); ?>" alt="Preview" class="max-h-40 mx-auto rounded-lg shadow-lg">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-section border-t pt-6">
                            ...
                            <div class="transform transition-all duration-300 hover:scale-105">
                                <textarea name="catatan" 
                                          rows="3"
                                          class="w-full px-4 py-3 border-2 border-blue-300 rounded-lg focus:outline-none focus:border-blue-700 transition-all"><?php echo htmlspecialchars($kontak['catatan'] ?? ''); ?></textarea>
                            </div>
                        </div>
